...
{	
	...
	/*  SETUP                                                               */
	/************************************************************************/
	setNumberOfRamTiles(70);
	memcpy (tiles, tileData, sizeof(tileData));  // copy tiles
	// setup initial sprite positions
	for (int n = 0; n < MAX_SPRITES; n++)
	{
		sprites[n].cx = ( 64 + (n % 8) * 24) << 6;
		sprites[n].cy =  (52 +(n / 8) * 24) << 6;
...
//			setLed(testLed); 
		...
		for (int i = 0; i < MAX_SPRITES; i++)
		{
			...
			putSprite(i, sprites[i].cx >> 6, sprites[i].cy >> 6, 0, 0);
		}
		drawSprites();
